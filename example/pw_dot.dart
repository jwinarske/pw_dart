...
Future<void> main(List<String> args) async {
  ...
  try {
    ...
    for (final node in g.nodes.values) {
      final inputs =
          g
              .portsForNode(node.id)
              .where((p) => p.direction == PwDirection.input)
              .toList();
      final outputs =
          g
              .portsForNode(node.id)
              .where((p) => p.direction == PwDirection.output)
              .toList();
      final inLabels = inputs
          .map((p) => '<i${p.id}> ${_esc(p.name)}')
          .join('|');
      final outLabels = outputs
          .map((p) => '<o${p.id}> ${_esc(p.name)}')
          .join('|');
      final body = [
    ...
    for (final link in g.links.values) {
      out.writeln(
        '  n${link.outputNodeId}:o${link.outputPortId} -> '
