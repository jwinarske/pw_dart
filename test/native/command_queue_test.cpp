...
TEST(CommandQueueTest, VariantTypeDiscrimination) {
    ...
    EXPECT_TRUE(q.try_push(CreateLinkCmd{.output_port_id = 1, .input_port_id = 2, .request_id = 100}));
    EXPECT_TRUE(q.try_push(DestroyLinkCmd{.link_id = 5, .request_id = 101}));
    EXPECT_TRUE(q.try_push(SetParamCmd{.node_id = 3, .param_json = R"({"key":"vol","value":0.5})", .request_id = 102}));
