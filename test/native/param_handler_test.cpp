...
struct glz::meta<ParamEntry::Flags> {
  ...
  static constexpr auto value =
      object("readable", &T::readable, "writable", &T::writable);
};

template <>
struct glz::meta<ParamEntry> {
  using T = ParamEntry;
  static constexpr auto value = object("key",
                                       &T::key,
                                       "value",
                                       &T::value,
                                       "type",
...
TEST(ParamHandlerTest, ParamArrayJson) {
  ...
  params.push_back(
      {.key = "name", .value = "\"Main Output\"", .type = "String"});
...
TEST(ParamHandlerTest, UnknownFieldsSkipped) {
  ...
  std::string json =
      R"({"key":"vol","value":"1.0","type":"Float","flags":{"readable":true,"writable":false},"unknown_field":"ignored"})";
  ParamEntry entry;
