...
void main() {
  group('PwGraph', () {
    ...
    test('fromJson with all object types', () {
      final json = {
        'nodes': [
          {'id': 1, 'name': 'node1', 'media_class': 'Audio/Sink', 'state': 'running'},
          {'id': 2, 'name': 'node2', 'media_class': 'Audio/Source', 'state': 'idle'},
        ],
        'ports': [
        ...
        'links': [
          {'id': 100, 'output_node_id': 2, 'output_port_id': 20, 'input_node_id': 1, 'input_port_id': 10, 'state': 'active'},
        ],
        'devices': [
          {'id': 200, 'name': 'ALSA'},
        ],
    ...
    test('fromJsonString', () {
      const jsonStr = '{"nodes":[{"id":1,"name":"n","state":"idle"}],"ports":[],"links":[],"devices":[]}';
    ...
    test('applyEvent NodeAdded', () {
      const graph = PwGraph.empty();
      final node = PwNode(id: 1, name: 'test', mediaClass: 'Audio/Sink', state: PwNodeState.idle);
    ...
    test('applyEvent NodeRemoved', () {
      final graph = PwGraph(nodes: {
        1: PwNode(id: 1, name: 'test', mediaClass: '', state: PwNodeState.idle),
      });
    ...
    test('applyEvent NodeInfoChanged updates existing node', () {
      final graph = PwGraph(nodes: {
        1: PwNode(id: 1, name: 'old', mediaClass: 'Audio/Sink', state: PwNodeState.idle),
      });
      final updatedNode = PwNode(id: 1, name: 'new', mediaClass: 'Audio/Sink', state: PwNodeState.running);
    ...
    test('applyEvent PortAdded', () {
      const graph = PwGraph.empty();
      final port = PwPort(id: 10, nodeId: 1, name: 'FL', direction: PwDirection.output);
      final updated = graph.applyEvent(PortAdded(port: port));
      expect(updated.ports.length, 1);
    });

    test('applyEvent PortRemoved', () {
      final graph = PwGraph(ports: {
        10: PwPort(id: 10, nodeId: 1, name: 'FL', direction: PwDirection.output),
      });
    ...
    test('applyEvent LinkAdded', () {
      const graph = PwGraph.empty();
      final link = PwLink(
        id: 100, outputNodeId: 1, outputPortId: 10,
        inputNodeId: 2, inputPortId: 20, state: PwLinkState.active,
      );
    ...
    test('applyEvent LinkStateChanged', () {
      final graph = PwGraph(links: {
        100: PwLink(
          id: 100, outputNodeId: 1, outputPortId: 10,
          inputNodeId: 2, inputPortId: 20, state: PwLinkState.init,
        ),
      });
    ...
    test('applyEvent ParamChanged does not modify graph topology', () {
      ...
      final updated = graph.applyEvent(const ParamChanged(nodeId: 1, key: 'vol'));
      expect(identical(updated, graph), isTrue);
    });

    test('portsForNode', () {
      final graph = PwGraph(
        nodes: {1: PwNode(id: 1, name: 'n', mediaClass: '', state: PwNodeState.idle)},
        ports: {
          10: PwPort(id: 10, nodeId: 1, name: 'a', direction: PwDirection.output),
          11: PwPort(id: 11, nodeId: 1, name: 'b', direction: PwDirection.output),
          20: PwPort(id: 20, nodeId: 2, name: 'c', direction: PwDirection.input),
        },
      );
    ...
    test('linksForNode', () {
      final graph = PwGraph(
        ports: {
          10: PwPort(id: 10, nodeId: 1, name: 'a', direction: PwDirection.output),
          20: PwPort(id: 20, nodeId: 2, name: 'b', direction: PwDirection.input),
        },
        links: {
          100: PwLink(id: 100, outputNodeId: 1, outputPortId: 10, inputNodeId: 2, inputPortId: 20, state: PwLinkState.active),
          101: PwLink(id: 101, outputNodeId: 3, outputPortId: 30, inputNodeId: 4, inputPortId: 40, state: PwLinkState.active),
        },
      );
    ...
    test('toJson round-trip', () {
      final graph = PwGraph(
        nodes: {1: PwNode(id: 1, name: 'n', mediaClass: 'Audio/Sink', state: PwNodeState.running)},
        ports: {10: PwPort(id: 10, nodeId: 1, name: 'p', direction: PwDirection.output)},
        links: {100: PwLink(id: 100, outputNodeId: 1, outputPortId: 10, inputNodeId: 2, inputPortId: 20, state: PwLinkState.active)},
