...
static void on_core_done(void* data, uint32_t id, int seq) {
  ...
  if (client)
    client->on_core_done_event(id, seq);
}

static void on_core_error(void* data,
                          uint32_t id,
                          int seq,
                          int res,
                          const char* message) {
...
bool PwDartClientImpl::connect(const char* remote_name) {
  ...
  // Connect to PipeWire
  auto* props =
      remote_name ? pw_properties_new(PW_KEY_REMOTE_NAME, remote_name, nullptr)
                  : nullptr;
  ...
  // Start the PW thread
  loop_thread_ =
      std::jthread([this](const std::stop_token& st) { this->loop_func(st); });
...
int32_t PwDartClientImpl::create_link(uint32_t output_port_id,
                                      uint32_t input_port_id) {
  if (!connected_.load(std::memory_order_acquire)) {
    return -1;
  }
  CreateLinkCmd cmd{.output_port_id = output_port_id,
...
int32_t PwDartClientImpl::set_node_param(uint32_t node_id,
                                         const char* param_json) {
  if (!connected_.load(std::memory_order_acquire) || !param_json) {
    return -1;
  }
  SetParamCmd cmd{
      .node_id = node_id, .param_json = param_json, .request_id = 0};
  return cmd_queue_.try_push(Command{cmd}) ? 0 : -2;
}
...
void PwDartClientImpl::loop_func(const std::stop_token& stop_token) {
  (void)stop_token;
