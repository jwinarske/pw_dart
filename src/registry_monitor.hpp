...
public:
    ...
    void on_port_info(uint32_t id, const struct pw_port_info* info);

    /// Called when link info changes.
    void on_link_info(uint32_t id, const struct pw_link_info* info);

    /// Called when device info changes.
    void on_device_info(uint32_t id, const struct pw_device_info* info);
...
private:
    ...
    static std::string dict_get(const struct spa_dict* dict, const char* key);

    /// Helper to extract all properties from spa_dict.
    static std::map<std::string, std::string> dict_to_map(const struct spa_dict* dict);
};
