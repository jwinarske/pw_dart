...
RegistryMonitor::RegistryMonitor(PwDartClientImpl* client, pw_registry* registry)
...
std::string RegistryMonitor::get_snapshot_json() {
    ...
    for (auto& [_, n] : nodes_) snap.nodes.push_back(n);
    for (auto& [_, p] : ports_) snap.ports.push_back(p);
    for (auto& [_, l] : links_) snap.links.push_back(l);
    for (auto& [_, d] : devices_) snap.devices.push_back(d);
...
void RegistryMonitor::on_global(uint32_t id, uint32_t permissions,
                                 const char* type, uint32_t version,
                                 const struct spa_dict* props) {
    (void)permissions;
    ...
    if (strcmp(type_str, PW_TYPE_INTERFACE_Node) == 0) {
        ...
        if (pd->proxy) {
            ...
            pw_node_add_listener(
                reinterpret_cast<pw_node*>(pd->proxy),
        ...
        // Initial info from props
        {
            ...
            if (info.name.empty()) info.name = dict_get(props, PW_KEY_NODE_DESCRIPTION);
        ...
        if (pd->proxy) {
            ...
            pw_port_add_listener(
                reinterpret_cast<pw_port*>(pd->proxy),
...

        {
            ...
            info.node_id = node_id_str.empty() ? 0 : static_cast<uint32_t>(std::stoul(node_id_str));
        ...
        if (pd->proxy) {
            ...
            pw_link_add_listener(
                reinterpret_cast<pw_link*>(pd->proxy),
...

        {
            ...
            info.output_node_id = onode.empty() ? 0 : static_cast<uint32_t>(std::stoul(onode));
            info.output_port_id = oport.empty() ? 0 : static_cast<uint32_t>(std::stoul(oport));
            info.input_node_id = inode.empty() ? 0 : static_cast<uint32_t>(std::stoul(inode));
            info.input_port_id = iport.empty() ? 0 : static_cast<uint32_t>(std::stoul(iport));
            info.state = "init";
            info.properties = dict_to_map(props);
            links_[id] = info;
        }
        ...
        if (pd->proxy) {
            ...
            pw_device_add_listener(
                reinterpret_cast<pw_device*>(pd->proxy),
...
void RegistryMonitor::on_global_remove(uint32_t id) {
    ...
    if (type_it == id_types_.end()) return;
...
void RegistryMonitor::on_node_info(uint32_t id, const struct pw_node_info* info) {
    if (!info) return;
    ...
    bool is_new = false;
    {
        ...
        // Update state
        switch (info->state) {
            case PW_NODE_STATE_ERROR:     node.state = "error"; break;
            case PW_NODE_STATE_CREATING:  node.state = "creating"; break;
            case PW_NODE_STATE_SUSPENDED: node.state = "suspended"; break;
            case PW_NODE_STATE_IDLE:      node.state = "idle"; break;
            case PW_NODE_STATE_RUNNING:   node.state = "running"; break;
        }
        ...
        if (info->props) {
            ...
            if (!mc.empty()) node.media_class = mc;
        }
...
void RegistryMonitor::on_port_info(uint32_t id, const struct pw_port_info* info) {
    if (!info) return;
...
void RegistryMonitor::on_link_info(uint32_t id, const struct pw_link_info* info) {
    if (!info) return;

    {
        ...
        switch (info->state) {
            case PW_LINK_STATE_ERROR:       link.state = "error"; break;
            case PW_LINK_STATE_UNLINKED:    link.state = "unlinked"; break;
            case PW_LINK_STATE_INIT:        link.state = "init"; break;
            case PW_LINK_STATE_NEGOTIATING: link.state = "negotiating"; break;
            case PW_LINK_STATE_ALLOCATING:  link.state = "allocating"; break;
            case PW_LINK_STATE_PAUSED:      link.state = "paused"; break;
            case PW_LINK_STATE_ACTIVE:      link.state = "active"; break;
        }
...
void RegistryMonitor::on_device_info(uint32_t id, const struct pw_device_info* info) {
    if (!info) return;

    {
        ...
        if (info->props) {
            ...
            if (!desc.empty()) device.description = desc;
        }
    }
}
...
std::string RegistryMonitor::dict_get(const struct spa_dict* dict, const char* key) {
    if (!dict || !key) return "";
    const char* val = spa_dict_lookup(dict, key);
    return val ? std::string(val) : "";
}

std::map<std::string, std::string> RegistryMonitor::dict_to_map(const struct spa_dict* dict) {
