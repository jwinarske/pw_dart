...
void ParamHandler::enum_params(uint32_t node_id) {
    if (!client_ || !client_->registry_monitor()) return;
...
std::string ParamHandler::get_cached_params_json(uint32_t node_id) {
    ...
    for (auto& [key, value] : it->second.params) {
        ...
        std::string type = (type_it != it->second.types.end()) ? type_it->second : "String";

        json += R"({"key":")" + key + R"(","value":)" + value +
                R"(,"type":")" + type + R"(","flags":{"readable":true,"writable":true}})";
    }
    json += "]";
    return json;
}

void ParamHandler::update_cache(uint32_t node_id, const std::string& key,
