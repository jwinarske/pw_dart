...
LinkManager::LinkManager(PwDartClientImpl* client) : client_(client) {}

glz::expected<uint32_t, LinkError> LinkManager::create_link(
    uint32_t output_port_id,
    uint32_t input_port_id) {
  if (!client_ || !client_->is_connected()) {
    return glz::unexpected(LinkError::NotConnected);
  }
  ...
  if (!props) {
    return glz::unexpected(LinkError::CreateFailed);
  }
...
glz::expected<void, LinkError> LinkManager::destroy_link(uint32_t link_id) {
  if (!client_ || !client_->is_connected()) {
    return glz::unexpected(LinkError::NotConnected);
  }
  ...
  if (result < 0) {
    return glz::unexpected(LinkError::DestroyFailed);
  }
