...
uint32_t pw_dart_get_pw_library_version(void) {
  ...
  unsigned long major = 0, minor = 0, micro = 0;
  if (ver) {
    char* end = nullptr;
    major = std::strtoul(ver, &end, 10);
    if (end && *end == '.') {
      minor = std::strtoul(end + 1, &end, 10);
    }
    if (end && *end == '.') {
      micro = std::strtoul(end + 1, &end, 10);
    }
  }
  return static_cast<uint32_t>((major << 16) | (minor << 8) | micro);
}
